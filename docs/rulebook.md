...
FLOORFOX must make its navigation behavior clear. Users may expect a live-navigation application to track their position and update the route while they walk, but FLOORFOX does not provide this functionality.

### 6.1

Before or when the first route is shown, the interface must clearly state in plain language:

> FLOORFOX shows a route and directions from your selected start to your selected destination. It does not track your live location or update your position while you walk.

The notice must be visible without opening a help page, must not be hidden behind a tooltip, and must not cover the map or route. It may be shortened in later sessions only if the full explanation remains readily accessible.

### 6.2

The interface must not use live-navigation language or controls that imply location tracking. It must not display a moving position marker, live distance-to-destination, automatic rerouting, arrival detection, or progress based on the user's location.

The interface must distinguish clearly between:

- the user's selected start zone;
- the selected destination zone;
- the route generated by FLOORFOX; and
- the user's actual position, which FLOORFOX does not know.

### 6.3

The result view must present the route on the building plan together with concise instructions and enough orientation information for the user to understand the route independently. If the user needs to correct their position or choose a new destination, they must start a new request manually.

### 6.4

Any feedback prompt must reinforce this limitation when necessary. Users may report that the displayed route or instructions were unclear, but FLOORFOX must never ask them to confirm an automatically detected arrival or imply that it observed their journey.

### 6.5

Room-number and named-destination inputs must accept normal typing and provide a clear example such as `E.62`. Search suggestions should appear only when useful, should match official room numbers, room titles, facilities, and approved common terms, and should remain limited to a short relevant result set. The interface must not require the user to browse a complete list of rooms. Input validation must tolerate harmless differences such as surrounding spaces and letter case where this does not create ambiguity, while preserving the official room number in displayed results. The interface must not silently replace an entered room number or named destination with a different place.

### 6.6

The `Popular destinations` chooser is a convenience feature, not the primary navigation control. The two main inputs remain free-text fields for start and destination. The curated menu must use familiar names, show no more than a small number of choices, and be easy to ignore. It must not open automatically, cover the map, or force the user through several steps for an ordinary room-number search.
